...
class YesSpider(scrapy.Spider):
    ...
    def start_requests(self):
        base_urls = (
            "https://www.yes24.com/24/category/bestseller?CategoryNumber=001001003&sumgb=09&year=2023&month="
            + YesSpider.month
            + "&PageNumber="
        )
    ...
    def parse(self, response, page_idx):
        ...
        # #category_layout > tbody > tr:nth-child(5) > td.goodsTxtInfo > p:nth-child(1) > a:nth-child(1)
        urls = response.css(
            "td.goodsTxtInfo > p:nth-child(1) > a:nth-child(1)::attr(href)"
        ).getall()
    ...
    def infoParse(self, response, rank, url):
        try:
            sub_title = response.css(
                "#yDetailTopWrap > div.topColRgt > div.gd_infoTop > div > h3::text"
            ).get()
        except:
            sub_title = "부제가 없습니다~!"
